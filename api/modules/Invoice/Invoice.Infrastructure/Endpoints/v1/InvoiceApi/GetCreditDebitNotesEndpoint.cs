...
public static class GetCreditDebitNotesEndpoint
{
    internal static RouteHandlerBuilder MapGetCreditDebitNotesEndpoint(this IEndpointRouteBuilder endpoints)
    {
        return endpoints
            .MapGet(
                "/credit-debit-notes",
                async (IInvoiceService service, [FromQuery] int page = 1, [FromQuery] int pageSize = 10, [FromQuery] decimal? sequenceNumber = null) =>
                {
                    var response = await service.GetCreditDebitNotes(page, pageSize, sequenceNumber);
